...
async function generateReport() {
  ...
  const checkBucket = async (bucket) => {
    ...
    for (const file of files) {
      if (file.metadata && file.metadata.size > 0.3 * 1024 * 1024) { // > 0.3MB
  ...
  let md = `# Báo Cáo: Các Hình Ảnh Đang Sử Dụng Có Dung Lượng Lớn (>0.3MB)\n\n`;
  ...
  if (largeFiles.length === 0) {
    md += `**Tuyệt vời! Không có bức ảnh nào đang sử dụng nặng trên 0.3MB.**\n`;
  } else {
